      ...
      >
        ...
        <template v-slot:default="dialog">
          <v-card>
            ...
            >Text Rank</v-toolbar>
            <v-card-text>
              <div class="pa-12" v-html="text_rank1"></div>
              <v-img   class="img" height="70px" src="@/assets/image4.png"/>
              <div class="pa-12" v-html="text_rank2"></div>
            </v-card-text>
            <v-card-actions class="justify-end">
              <v-btn
                text
...
"<p>TF&nbsp;<em>термина а</em>&nbsp;= (Количество раз, когда термин&nbsp;<span>а</span>&nbsp;встретился в тексте / количество всех слов в тексте)**</p>",
            ...
            text_rank1:"<p><span>TextRank - графовый алгоритм, главной идеей которого является</span><br /><span>&ldquo;рекомендация&rdquo;. Каждое предложение выполняет роль вершины</span><br /><span>направленного графа. Если вершина A соединяется с вершиной B, это значит, что B получила &ldquo;голос&rdquo; или &ldquo;рекомендацию&rdquo; от A. Чем больше число голосов у вершины, тем больше важность предложения, более того, важность вершины, которая отправляет голос, влияет на важность голоса.</span><br /><span>Рекомендацию можно определить по-разному. Для задачи</span><br /><span>суммаризации такой &ldquo;голос&rdquo; определяют, как сходство двух предложений. В оригинальном предложенном алгоритме, сходство предложений вычисляется как нормализованное пересечение этих предложений.</span></p>",
            text_rank2:"<p><span>Алгоритм суммаризации основанный на этом методе будет выглядеть</span><br /><span>следующим образом:</span><br /><span>1. загрузить текстовый документ;</span><br /><span>2. предобработать текст;</span><br /><span>3. составить граф, вершинами которого будут являться</span><br /><span>предложения;</span><br /><span>4. получить оценки сходства для каждой пары предложений;</span><br /><span>5. отсортировать все вершины и составить краткое содержание.</span></p>"
        }
    },
 computed:{
      ...mapState({
